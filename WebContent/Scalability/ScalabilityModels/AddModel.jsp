...
<% 
	String redirectURL = "../ScalabilityModels.jsp";
	...
	else
	{
		...
		try {
			...
			// remove duplicates
			String delete = "DELETE from `cbsa`.`scalabilitymodel` WHERE `scalabilitymodel`.`Name` = '" + request.getParameter("name").replaceAll(" ", "") + "' AND `scalabilitymodel`.`Type` <> 'System Defined'";
			PreparedStatement removeDups = connection.prepareStatement(delete);
			removeDups.executeUpdate();
			
			// insert model info
			String insertModel = "INSERT INTO  `scalabilitymodel` (  `ModelID` ,  `isDefault` ,  `Name` ,  `Description` ,  `NumMetrics` ,  `Type` ,  `Status` ,  `value` )" 
			...
			String insertedModel = "SELECT ModelID FROM scalabilitymodel WHERE Name = '" + request.getParameter("name").replaceAll(" ", "") + "'";
			...
			if(resultsetmodel.next())
			{
				...
				for(int i = 1; i <= Integer.parseInt(request.getParameter("numMetrics")); i++)
				{
					...
					String checkUserDef = "SELECT distinct Name FROM scalabilitymetric WHERE Name = '" + request.getParameter("type" + i) + "' AND Type = 'System Defined'";
					...
					if(resultsetUserDef.next())
						getMetricId = "SELECT MetricID FROM scalabilitymetric WHERE Name = '" + request.getParameter("type" + i) + "' AND Statistic = '" + request.getParameter("statistic" + i) + "'";
					else
						getMetricId = "SELECT MetricID FROM scalabilitymetric WHERE Name = '" + request.getParameter("type" + i) + "' AND Statistic = 'None'";
					...
					{	
						...
						// add to modelmetric
						String insertModelMetric = "INSERT INTO  `scalabilitymodelmetric` (  `ModelID` ,  `MetricID` , `ModelMetric`)" 
				...
				// add model's metric
				String insertMetric = "INSERT INTO  `scalabilitymetric` (  `MetricID` ,  `isDefault` ,  `Name` ,  `Statistic` ,  `Description` ,  `Equation` ,  `Type` )" 
				...
				String insertedMetric = "SELECT MetricID FROM scalabilitymetric WHERE Name = '" + request.getParameter("name").replaceAll(" ", "") + "'";
				...
				if(ok)
					redirectURL = "../ScalabilityModels.jsp?message=" + "The model was successfully created.";
				else
